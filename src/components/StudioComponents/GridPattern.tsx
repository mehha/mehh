...
function Block({
  ...
  const [color, setColor] = useState('rgba(0, 0, 0, 0.2)') // Initial color with 50% opacity

  // Generate a random color with opacity on initial render
  useEffect(() => {
    const randomColor = `rgba(${Math.floor(Math.random() * 256)}, ${Math.floor(Math.random() * 256)}, ${Math.floor(Math.random() * 256)}, 0.2)` // 0.5 opacity
