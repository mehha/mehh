...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`pages_blocks_archive\` (
  	...
  	\`is_module_style\` integer DEFAULT 0,
  	\`enable_link\` integer,
  	\`link_type\` text DEFAULT 'reference',
  	\`link_new_tab\` integer,
  	\`link_url\` text,
  ...
  await db.run(sql`CREATE TABLE \`pages_blocks_services_block_service_items\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`rich_text\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`pages_blocks_services_block_service_items_order_idx\` ON \`pages_blocks_services_block_service_items\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`pages_blocks_services_block_service_items_parent_id_idx\` ON \`pages_blocks_services_block_service_items\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`pages_blocks_services_block_media_idx\` ON \`pages_blocks_services_block\` (\`media_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_pages_v_blocks_services_block_service_items\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`rich_text\` text,
  	\`stack\` text,
  	\`_uuid\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`_pages_v_blocks_services_block\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_services_block_service_items_order_idx\` ON \`_pages_v_blocks_services_block_service_items\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_services_block_service_items_parent_id_idx\` ON \`_pages_v_blocks_services_block_service_items\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_pages_v_blocks_services_block\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_services_block_order_idx\` ON \`_pages_v_blocks_services_block\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_services_block_parent_id_idx\` ON \`_pages_v_blocks_services_block\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_services_block_path_idx\` ON \`_pages_v_blocks_services_block\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_pages_v_blocks_services_block_media_idx\` ON \`_pages_v_blocks_services_block\` (\`media_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`posts_media_idx\` ON \`posts\` (\`media_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_posts_v\` (
  	...
  	\`version_homepage\` text,
  	\`version_year\` text,
  	\`version_client\` text,
  	\`version_service\` text,
  	\`version_meta_title\` text,
  	\`version_meta_image_id\` integer,
  	\`version_meta_description\` text,
  	\`version_published_at\` text,
  ...
  await db.run(sql`CREATE INDEX \`_posts_v_version_version_media_idx\` ON \`_posts_v\` (\`version_media_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`pages_blocks_services_block_service_items\`;`)
  ...
  await db.run(sql`DROP TABLE \`_pages_v_blocks_services_block_service_items\`;`)
  await db.run(sql`DROP TABLE \`_pages_v_blocks_services_block\`;`)
